class MainController < ApplicationController
  skip_before_filter :verify_authenticity_token, :only => [:organization_form_submission]
  def home
  ...
  def fetch
    @organizations = Organization.where(approved: true).order('name asc').paginate(page: params[:page], per_page: 150)
